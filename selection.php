				...
				<?
					...
					while ($i < $perPage && $row = mysqli_fetch_array($res)){
						if ($row["timeStamp"] > $r){
					 		$tooltip = $row["comment"];
							if ($row["money"]<0){ 		
					 			echo '<div class="row the-tooltip top right sky-blue" style="background: none repeat scroll 0% 0% #FF905A;"><div class="cell">', $row["name"], '</div><div class="cell">', $row["money"], /*"</div><td>, $row["type"], </td>",*/'</div><div class="cell">', $row["dateTime"], '</div><div class="cell" style="display:none;">', $row["timeStamp"], '</div><div class="cell" style="display:none;">', $row["id"], '</div><div>'.$tooltip.'</div></div>';	
					 		} else {
